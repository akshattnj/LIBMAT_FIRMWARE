...
namespace EC20
{
    ...
    void commandControl(void *args)
    {
        ...
        while (1)
        {
            ...
            if(esp_timer_get_time() - delayMQTT > MQTTDelay && ((flagsEC20 & BIT2) != 0))
            {
                ...
                if(flagsEC20 & BIT6)
                        sprintf(cmd.data, "{\"ba%%\":%d,\"baV\":%0.2f,\"lat\":%0.5f,\"lon\":%0.5f, \"temp\":%0.2f, \"humidity\":%0.2f}", Commons::batteryPercentage, Commons::batteryVoltage, Commons::latitude, Commons::longitude, AHT::temperature, AHT::humidity);
                else
                    sprintf(cmd.data, "{\"ba%%\":%d,\"baV\":%0.2f, \"temp\":%0.2f, \"humidity\":%0.2f}", Commons::batteryPercentage, Commons::batteryVoltage, AHT::temperature, AHT::humidity);
                cmd.command = 0x02;
    ...
    */
    void reconnectMQTT()
    {
        ...
        sendATCommand(messageBuffer, dataLen, true);
        ESP_LOGI(EC20_TAG, "Connecting to Thingsboard");
        if(!waitForMQTT(10000 / portTICK_PERIOD_MS)){
    ...
    void readGPSData(char *output)
    {
        ...
        while (ptr != NULL) 
        {
            ...
            if (commaCount == 4) 
            {
                ...
                Commons::latitude = latDegree + latMinute;
                if (Commons::latitude < -90 || Commons::latitude > 90) 
                {
            ...
            else if (commaCount == 6) 
            {
                ...
                Commons::longitude = lonDegree + lonMinute;
                if (Commons::longitude < -180 || Commons::longitude > 180) 
                {
        ...
        if(Commons::longitude == 0.00 && Commons::latitude == 0.00)
        {
    ...
    */
    ...
    bool waitForMQTT(uint32_t timeout)
    {
        ...
        {
            ESP_LOGI(EC20_TAG, "MQTT response received");
            return true;
        }
        else
        {
            ESP_LOGI(EC20_TAG, "MQTT response timed out");
            return false;
        }
    }
