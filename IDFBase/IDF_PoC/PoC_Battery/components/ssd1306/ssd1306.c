...
void ssd1306_display_text_x4(SSD1306_t * dev, int page, char * text, int text_len, bool invert)
{
	if (page >= dev->_pages) return;
	int _text_len = text_len;
	if (_text_len > 5) _text_len = 5;
	...
	for (uint8_t nn = 0; nn < _text_len; nn++) {
		...
		// render character in 8 column high pieces, making them 4x as wide
		for (uint8_t yy = 0; yy < 24; yy += 8)	{ // for each group of 8 pixels high (y-direction)
			uint8_t image[32];
			for (uint8_t xx = 0; xx < 8; xx++) { // for each column (x-direction)
				image[xx*4+0] = 
				image[xx*4+1] = 
				image[xx*4+2] =
				image[xx*4+3] = out_columns[xx].u8[yy/8];
			}
			if (invert) ssd1306_invert(image, 32);
			if (dev->_flip) ssd1306_flip(image, 32);
			if (dev->_address == SPIAddress) {
				spi_display_image(dev, page+(yy/8), seg, image, 32);
			} else {
				i2c_display_image(dev, page+(yy/8), seg, image, 32);
			}
			memcpy(&dev->_page[page+(yy/8)]._segs[seg], image, 32);
		}
		seg = seg + 32;
	}
}
